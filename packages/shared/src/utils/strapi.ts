...
import { API_HOST } from '@web/shared/apis';
import { IAnyObject } from '@web/shared/types/etc';
import { IFile } from '@web/shared/types/strapi/file';

type TFormat = 'original' | 'large' | 'medium' | 'small' | 'thumbnail';
 ...
 */
export const getFileURL = (mediaItem: IFile, format: TFormat = 'original') => {
  // 미디어가 없다면 아래 메시지 반환
  if (!mediaItem) {
  ...
  if (
    ...
    return API_HOST + mediaItem.url;
  }

  // 입력받은 포맷에 맞는 포맷이 있다면 그 포맷으로 반환
  return API_HOST + mediaItem.formats[format].url;
};
 ...
 * - mediaItem: strapi media item
 */
export const getFile = async (mediaItem: IFile) => {
...
// export const replacePlaceholderMessage = (string, message) => {
